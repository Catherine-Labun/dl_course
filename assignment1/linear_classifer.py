...
def l2_regularization(W, reg_strength):
    ...
    l2_reg_loss = reg_strength*np.sum(W*W)
    loss = l2_reg_loss
...
class LinearSoftmaxClassifier():
    ...
    def fit(self, X, y, batch_size=100, learning_rate=1e-7, reg=1e-5,
        ...
        for epoch in range(epochs):
            ...
            for minibatch in batches_indices:
                
                loss1, dW = linear_softmax(X[minibatch], self.W, y[minibatch])
                loss2, grad = l2_regularization(self.W, reg)
                loss = loss1 + loss2
                dL_w = dW + grad
                self.W -= learning_rate*dL_w
